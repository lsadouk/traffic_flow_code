...
% detector is is 51.82)

% convert to miles -> for miles where no detector is found, take the State
% of previous mile
starting_day = '09/01/2017';
ending_day = '09/01/2017'; % was '09/30/2017'; 
% I5-north/south-D7 for peak hours 7to9am, we delete 11 at the end and 77 sendors
freeway = input('Please select the freeway: H101_North_D7 / H101_South_D7 / I5_North_D7 / I5_South_D7 / I5_North_D11 / I210_West_D7 / SR1_North' ,'s');
nb_days = daysact(starting_day, ending_day)+1;
nb_VDS_given = input('Please enter the number of VDSs (82_D07-US101N & 93_D07-US101S & 90_D07-I5N &  99_D07-I5S):  ');
%  nb_VDS_given = 17;  % 1. for D11 I5N 1 station located at 35.38 at 17H00 
%  nb_VDS_given = 24;  % 1. for D7 I270W 1 station located at 35.41 at 08H40 
%  nb_VDS_given = 12;  % 1. for D7 I270W 1 station located at 35.41 at 08H40 
...
%  nb_VDS_deleted_start = 3; % 1. for D07 I5N(training and testing)
%  nb_VDS_deleted_start = 0; % 1. for D07 US101s( testing)  / I5S
nb_VDS_deleted_end= input('Please enter the number of VDSs you wish to delete at the end:  ');
%  nb_VDS_deleted_end = 0; % 1. for D07 US101N(training and testing) / I5N 
...
%data =xlsread(strcat('../data/original_data/pems_output_',freeway, '_red_L35_T17h.xlsx'),day_count,'A:F'); 
data =xlsread(strcat('../data/original_data/pems_output_',freeway, '_red_L329.55_T15h05_wStr10_Pt',int2str(next_pred_point) ,'.xlsx'),day_count,'A:F'); 
%data =xlsread(strcat('../data/original_data/pems_output_',freeway, '_red_L35_T08h40_wStr10_Pt',int2str(next_pred_point) ,'.xlsx'),day_count,'A:F'); 
...
%     data_n_oneMile(:,:,3) = (data_n_oneMile(:,:,3) - min_flow)/ (max_flow-min_flow);
    ...
    h_stride = 10; % time interval
    w_stride = 10; % space interval
    ...
    for row = 1:h_data-h_stride+1
        for column = 1:w_data-w_stride+1-next_pred_point % was 1:w_data-w_stride+1 (i.e. 180-20+1=161)
            ...
            label(1,image_count) = data_n_oneMile(row, column+w_stride-1+next_pred_point, 1); % predictin the speed
...
filename = strcat('traffic_images_', freeway, '_', int2str(nb_days), 'days_',int2str(next_pred_point), 'Pt_L329.55_T15h05_10wStr.mat'); %'traffic_images_H101_South_D7_15days.mat'
